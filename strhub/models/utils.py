...
def _get_config(experiment: str, **kwargs):
    ...
    with open(root / f'configs/charset/229_full.yaml', 'r') as f:
...
def create_model(experiment: str, pretrained: bool = False, **kwargs):
    ...
    ModelClass, _ = _get_model_class(experiment)
    # print(**config)
    ...
    if pretrained:
        try:
            ...
            raise InvalidModelError("No pretrained weights found for '{}'".format(pretrained)) from None
...
def load_from_checkpoint(checkpoint_path: str, **kwargs):
    ...
    try:
        print("First option load checkpoint")
        ModelClass, _ = _get_model_class(checkpoint_path)
        model = ModelClass.load_from_checkpoint(checkpoint_path, **kwargs)
    except:
        print("Second option load checkpoint")
