...
import transformers
from strhub.multilingual_clip import pt_multilingual_clip
from strhub.clip import clip
from strhub.models.base import CrossEntropySystem
from .modules import DecoderLayer, Decoder, modify_attn_mask
import open_clip
from dict_trie import Trie
from strhub.data.gen_candidate import gen_candidate
# an alternative choice when the input argument is not valid 
CLIP_PATH = '/content/clip4str_inference/pretrained/clip'
M_CLIP_PATH = '/content/lip4str_inference/pretrained/M_clip'
...
class VL4STR(CrossEntropySystem):
    ...
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        self.freeze_language_backbone = self.text_freeze_nlayer >= 24
        self.freeze_image_backbone = self.image_freeze_nlayer >= 24
        ...
        self.dictionary = open("./strhub/data/syllables.txt","r",encoding = "utf-8").read().replace("\n\n", "\n").split("\n")
        ...
        #NOTE: Change here to load multiligual CLIP: use text encoder of multiplingual, vision encoder of original CLIP
        assert "clip_pretrained" in kwargs.keys()
        print(kwargs["clip_pretrained"])
        ...
        assert "M_clip_pretrained" in kwargs.keys()
        # if not os.path.exists(kwargs["M_clip_pretrained"]):
        #     kwargs["M_clip_pretrained"] = os.path.join(M_CLIP_PATH, os.path.basename(kwargs["M_clip_pretrained"]))
        #     assert os.path.exists(kwargs["M_clip_pretrained"])
        # load CLIP model
        clip_model, _ = clip.load(name=kwargs["clip_pretrained"], device=self._device)
        M_clip_model = pt_multilingual_clip.MultilingualCLIP.from_pretrained(kwargs["M_clip_pretrained"]).to(self._device)
        self.M_tokenizer = transformers.AutoTokenizer.from_pretrained(kwargs["M_clip_pretrained"])
        self.clip_model = clip_model.float()
        self.M_clip_model = M_clip_model.float()
        self.second_clip_model, _, preprocess = open_clip.create_model_and_transforms('xlm-roberta-base-ViT-B-32', pretrained="./pretrained/clip/vintext-1M/just_model_epoch_19.pt")
        self.clip_tokenizer = open_clip.get_tokenizer('xlm-roberta-base-ViT-B-32')
        ...
        self.M_clip_model.transformer.apply(lambda m: modify_attn_mask(m, context_length=self.context_length))
        ...
        vis_embed_dim = self.M_clip_model.LinearTransformation.weight.shape[0] if self.use_share_dim else self.clip_model.visual.proj.shape[0]
        ...
        # NOTE: tokenizer is coded in \strhub\data\utils.py, it has an internal function __len__()
        # funtion len() will return len of a tuple name self._itos that contains 3 special tokens and our charset
        # added by clip4str for their own encoder and decoder (not related with encoder from CLIP and M_clip)
        # for vietnamese, len(self.tokenizer) should return 232 (229 + 3), so let's print to check:
        ...
        if self.use_language_model:
            ...
            cross_embed_dim = self.M_clip_model.LinearTransformation.weight.shape[0]
    ...
    def encoder_cross_modal_feature(self, prev_logits, image_feat):
        ...
        #NOTE: now we will use self.M_clip_model to encode text from here
        ...
        #     text_features = self.clip_model.token_embedding(text) if self.cross_token_embeding else \
        #                             self.clip_model.encode_text(text)  
        # print("Text feature from clip ",text_features.shape)      
        text_features = self.M_clip_model(captions, self.M_tokenizer)
        # print("Image_feature shape", image_features.shape)
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None, is_train = True) -> Tensor:
        ...
        if self.use_language_model:
            ...
            if self.cross_fast_decode:
                ...
                if not is_train:
                  ...
                  image_features = self.second_clip_model.encode_image(images)
                  # if not self.use_share_dim:
                  #     image_features = torch.matmul(image_features, self.clip_model.visual.proj)
                  ...
                  probs = prev_logits.softmax(-1)
                  # adapt for the test charset, CLIP is not sensitive to uppercase or symbols
                  #NOTE: Check if decode_fast change probs
                  preds, probs = self.tokenizer.decode_fast(probs, charset_adapter=self.charset_adapter)
                  # print(preds)
                  texts = self.clip_tokenizer(preds, context_length=self.context_length).to(memory.device)

                  # return all text features
                  with torch.no_grad():
                      text_features = self.second_clip_model.encode_text(texts)
                  ...
                  # This is a tensor list contain n element, each element is the similarity of an image with its
                  # equavilent text which is prediction from visual decoder
                  logits_per_image = torch.sum(logits_per_image,dim = 1)
                  # print(logits_per_image)
                  for i,score in enumerate(logits_per_image):
                      #FIXME Choose random the number 30
                      if score > 306:
                        cross_logits[i] = logits[i]


        if self.refine_iters:
            print(self.refine_iters)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        self.log('loss', loss)
        with open(self.save_loss,"a",encoding="utf-8") as f:
            f.write(" " + str(loss))
        return loss
    ...
    def configure_optimizers(self):
        ...
        # NOTE: n means name of the layer, p means parameters of the layer
        # Author distinguish LayerNorm, Bias layer, Logit Scale, text_embedding layer with another layer
        # With exclude layers,they will have weight decay = 0 during training, others will have weight decay
        # as in config
        exclude = lambda n, p: p.ndim < 2 or "bn" in n or "ln" in n or "bias" in n or 'logit_scale' in n \
                            or "pos_queries" in n or "text_embed" in n or "word_embed" in n or "LayerNorm" in n \
                            or "LinearTransformation" in n 
        include = lambda n, p: not exclude(n, p)

        # encoder parameters
        encoder_params = list(self.clip_model.named_parameters())
        encoder_params = encoder_params +  list(self.M_clip_model.named_parameters())
        ...
        # decoder parameters
        decoder_params = [(n, p) for n, p in list(self.named_parameters()) if "clip_model" not in n and "M_clip_model" not in n]
        ...
        print("Total Steps:", self.trainer.estimated_stepping_batches, type(self.trainer.estimated_stepping_batches))
        print("pct_start: ", self.warmup_pct, type(self.warmup_pct))
        print("learning rate: ", lr, type(lr))
        sched = OneCycleLR(optimizer, [lr, lr, lr * self.coef_lr, lr * self.coef_lr],
                            total_steps = self.trainer.estimated_stepping_batches, pct_start=self.warmup_pct,
    ...
    def _freeze_backbones(self):
        ...
        for name, mod in self.clip_model.named_modules():
            ...
            #         mod.eval()
        for name, mod in self.M_clip_model.named_modules():
        ...
        if self.freeze_language_backbone:
            ...
            self.M_clip_model.transformer.eval()
            self.M_clip_model.LinearTransformation.eval()
    ...
    def freeze_cip_layers(self, image_freeze_nlayer, text_freeze_nlayer, image_freeze_layer_divisor=-1, image_only_fc=False):
        ...
        """
        assert image_freeze_nlayer <= 24 and text_freeze_nlayer <=24 and image_freeze_layer_divisor <= 24
        if hasattr(self, "clip_model") and hasattr(self, "M_clip_model"):
            ...
            if text_freeze_nlayer > -1:
                for name, param in self.M_clip_model.named_parameters():
                    # top layers always need to train
                    if name.startswith("LinearTransformation") or name.startswith("transformer.pooler"):
                        continue
                    elif name.startswith("transformer.encoder."):
                        if len(name.split(".layer.")) == 1:
                          continue
                        layer_num = int(name.split(".layer.")[1].split(".")[0])
            ...
            if text_freeze_nlayer >= 24:
                for n, p in self.M_clip_model.named_parameters():
                    # exclude visual parameters              
                    if "transformer" in n or "LinearTransformation" in n:
                        p.requires_grad = False
            
            if image_freeze_nlayer >= 24:
