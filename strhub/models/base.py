...
class BatchResult:
    ...
    loss: Tensor
    loss_numel: int
    # NOTE: Add predicts so that can visualize the results
    preds: list
    gts: list
...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool, clip_refine: bool = False, **kwargs) -> Optional[STEP_OUTPUT]:
        ...
        if validation:
            ...
            logits = self.forward(images, is_train = False)
            loss = loss_numel = None  # Only used for validation; not needed at test-time.

            # preds, probs, from_dict = self.forward(images, is_train = False)
            ...
            #     if preds[i] == labels[i]:
            #       acc_dict += 1
            #   print(100*acc_dict/len(from_dict))
            # return dict(output=BatchResult(total, correct, ned, confidence, label_length, loss, loss_numel,preds, labels))
